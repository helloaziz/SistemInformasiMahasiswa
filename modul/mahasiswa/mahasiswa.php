...
<head>
			...
			<div class="container">
				...
				<div class="collapse navbar-collapse" id="navbarSupportedContent">
					<ul class="navbar-nav mr-auto">
						...
						<li class="nav-item">
							<a class="nav-link" href="media.php?modul=mahasiswa&prosesmhs=tambahmhs">Tambah</a>
						</li>
						<li>
			...
			<table class="table table-striped table-hover table-sm table-bordered">
				...
				<?php
						...
						if(isset($_POST['tombolcari'])){
							...
							$sql = "SELECT * FROM mahasiswa WHERE nama like '%$keyword%' LIMIT ".$posisi.", ".$batas;		
						}else{
							$sql = "select * from mahasiswa LIMIT ".$posisi.", ".$batas;		
						}
						
						// Langkah 2. Sesuaikan query dengan posisi dan batas
						$sql = "SELECT * FROM mahasiswa ORDER BY id asc LIMIT $posisi,$batas";
							...
							while ($data=mysqli_fetch_array($tampil)){
								...
								<tr>
									...
									<td>
										...
										<a href="media.php?modul=mahasiswa&prosesmhs=koreksimhs&id='.$data['id'].'" class="badge badge-warning">Koreksi</a>
										
										<a href="media.php?modul=mahasiswa&prosesmhs=hapusmhs&id='.$data['id'].'" class="badge badge-danger" onclick="return confirm(\'Yakin ingin menghapus data ini?\')">Hapus</a>
				...
				<ul class="pagination">
					<?php
							...
							for($i=1;$i<=$jmlhalaman;$i++){
								if ($i != $halaman){
								echo "<li class='page-item'><a class='page-link' href='media.php?modul=mahasiswa&halaman=$i'>$i</a></li>";
								}
						else{ 
						echo "<li class='page-item active'><a class='page-link' href='#'>$i</a></li>";
						}
					}
					echo "<p>Total Mahasiswa : <b>$jmldata</b> Mahasiswa</p>";
					?>
				</ul>
			</div>
		...
		require ('functions.php'); 
			if(isset($_POST['submit'])){
				...
				if(mysqli_num_rows($cek) == 0){
					$sql = mysqli_query($koneksi, "INSERT INTO mahasiswa(nim, nama, jenis_kelamin, program_studi, foto)VALUES('$nim', '$nama', '$jenis_kelamin', '$program_studi', '$foto')") or die(mysqli_error($koneksi));
					
					if($sql){
						echo '<script>alert("Berhasil menambahkan data."); document.location="media.php?modul=mahasiswa";</script>';
					}else{
				...
				if(in_array($ekstensi, $ekstensifile_diperbolehkan) == true)  
					{
						...
						$query=mysqli_query($koneksi,"UPDATE mahasiswa SET nama = '$nama', jenis_kelamin = '$jenis_kelamin', program_studi='$program_studi', foto='$nama_gambar_baru' where id='$id'");
						...
						if(!$query)
							...
							{
								echo "<script>alert('Data berhasil diubah.');window.location='media.php?modul=mahasiswa';</script>";
							}
					} 
						else 
					{     
					echo "<script>alert('Ekstensi gambar yang boleh hanya jpg,jpeg,gif atau png.');window.location='media.php?modul=mahasiswa';</script>";
					}
			} 
			else 
			{
			$query = "UPDATE mahasiswa SET nama='".$nama."', jenis_kelamin='".$jenis_kelamin."', program_studi='".$program_studi."' WHERE id='".$id."'";
			...
			if($sql)
				{
					...
					echo '<script>alert("Berhasil menyimpan data."); document.location="media.php?modul=mahasiswa";</script>';
				}
							else
						{
		...
		<form action=" "?id=<?php echo $id; ?>" method="post" enctype="multipart/form-data">
			...
			<div class="form-group row">
				...
				<div class="col-sm-10">
					...
					<a href="media.php?modul=mahasiswa" class="btn btn-warning">KEMBALI</a>
				</div>
			</div>
		</form>
	</div>
	...
	if(isset($_GET['id'])){
		...
		if(mysqli_num_rows($cek) > 0){
			...
			if($del){
				echo '<script>alert("Berhasil menghapus data."); document.location="media.php?modul=mahasiswa";</script>';
			}else{
				echo '<script>alert("Gagal menghapus data."); document.location="modul=mahasiswa";</script>';
			}
		}else{
			echo '<script>alert("ID tidak ditemukan di database."); document.location="modul=mahasiswa";</script>';
		}
	}else{
		echo '<script>alert("ID tidak ditemukan di database."); document.location="modul=mahasiswa";</script>';
	}
...
<script src="modul/mahasiswa/skrip.js"></script>	
</body>
