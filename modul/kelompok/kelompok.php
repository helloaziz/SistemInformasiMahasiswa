...
<head>
	...
	<body>
			...
			<div class="container">
				...
				<div class="collapse navbar-collapse" id="navbarSupportedContent">
					<ul class="navbar-nav mr-auto">
						...
						<li class="nav-item">
							<a class="nav-link" href="media.php?modul=kelompok&prosesklp=tambahklp">Tambah</a>
						</li>
						<li>
			...
			<table class="table table-striped table-hover table-sm table-bordered">
				...
				<?php
						...
						if(isset($_POST['tombolcari'])){
							...
							$sql = "SELECT * FROM kelompok WHERE nama like '%$keyword%' LIMIT ".$posisi.", ".$batas;		
						}else{
							$sql = "select * from kelompok LIMIT ".$posisi.", ".$batas;		
						}
						
						// Langkah 2. Sesuaikan query dengan posisi dan batas
						$sql = "SELECT * FROM kelompok ORDER BY id asc LIMIT $posisi,$batas";
							...
							while ($data=mysqli_fetch_array($tampil)){
								...
								<tr>
									...
									<td>
										...
										<a href="media.php?modul=kelompok&prosesklp=koreksiklp&id='.$data['id'].'" class="badge badge-warning">Koreksi</a>
										
										<a href="media.php?modul=kelompok&prosesklp=hapusklp&id='.$data['id'].'" class="badge badge-danger" onclick="return confirm(\'Yakin ingin menghapus data ini?\')">Hapus</a>
		...
		<?php
				...
				$query2     = mysqli_query($koneksi, "select * from kelompok");
				...
				<ul class="pagination">
					<?php
							...
							for($i=1;$i<=$jmlhalaman;$i++){
								if ($i != $halaman){
								echo "<li class='page-item'><a class='page-link' href='media.php?modul=kelompok&halaman=$i'>$i</a></li>";
								}
						else{ 
						echo "<li class='page-item active'><a class='page-link' href='#'>$i</a></li>";
						}
					}
					echo "<p>Total Kelompok : <b>$jmldata</b> Kelompok</p>";
					?>
				</ul>
			</div>
	...
	<?php
		require ('functions.php'); 
			if(isset($_POST['submit'])){
				...
				$cek = mysqli_query($koneksi, "SELECT * FROM kelompok WHERE id='$id'") or die(mysqli_error($koneksi));
				...
				if(mysqli_num_rows($cek) == 0){
					$sql = mysqli_query($koneksi, "INSERT INTO kelompok(nim, nama, jenis_kelamin, program_studi, foto)VALUES('$nim', '$nama', '$jenis_kelamin', '$program_studi', '$foto')") or die(mysqli_error($koneksi));
					
					if($sql){
						echo '<script>alert("Berhasil menambahkan data."); document.location="media.php?modul=kelompok";</script>';
					}else{
	...
	?>
			...
			<form action="" method="post" enctype="multipart/form-data">
				...
				<div class="form-group row">
					<label class="col-sm-2 col-form-label">NAMA ANGGOTA KELOMPOK</label>
					<div class="col-sm-10">
	...
	<div class="container" style="margin-top:20px">
				...
				if(in_array($ekstensi, $ekstensifile_diperbolehkan) == true)  
					{
						...
						$query=mysqli_query($koneksi,"UPDATE kelompok SET nama = '$nama', jenis_kelamin = '$jenis_kelamin', program_studi='$program_studi', foto='$nama_gambar_baru' where id='$id'");
						...
						if(!$query)
							...
							{
								echo "<script>alert('Data berhasil diubah.');window.location='media.php?modul=kelompok';</script>";
							}
					} 
						else 
					{     
					echo "<script>alert('Ekstensi gambar yang boleh hanya jpg,jpeg,gif atau png.');window.location='media.php?modul=kelompok';</script>";
					}
			} 
			else 
			{
			$query = "UPDATE kelompok SET nama='".$nama."', jenis_kelamin='".$jenis_kelamin."', program_studi='".$program_studi."' WHERE id='".$id."'";
			...
			if($sql)
				{
					...
					echo '<script>alert("Berhasil menyimpan data."); document.location="media.php?modul=kelompok";</script>';
				}
							else
						{
		...
		<?php
			...
			if(isset($_GET['id']))
			{
				...
				$select = mysqli_query($koneksi, "SELECT * FROM kelompok WHERE id='$id'") or die(mysqli_error($koneksi));
		...
		<form action=" "?id=<?php echo $id; ?>" method="post" enctype="multipart/form-data">
			...
			<div class="form-group row">
				...
				<div class="col-sm-10">
					...
					<a href="media.php?modul=kelompok" class="btn btn-warning">KEMBALI</a>
				</div>
			</div>
		</form>
	</div>
	...
	if(isset($_GET['id'])){
		...
		if(mysqli_num_rows($cek) > 0){
			...
			$del = mysqli_query($koneksi, "DELETE FROM kelompok WHERE id='$id'") or die(mysqli_error($koneksi));
			if($del){
				echo '<script>alert("Berhasil menghapus data."); document.location="media.php?modul=kelompok";</script>';
			}else{
				echo '<script>alert("Gagal menghapus data."); document.location="modul=kelompok";</script>';
			}
		}else{
			echo '<script>alert("ID tidak ditemukan di database."); document.location="modul=kelompok";</script>';
		}
	}else{
		echo '<script>alert("ID tidak ditemukan di database."); document.location="modul=kelompok";</script>';
	}
...
<script src="modul/kelompok/skrip.js"></script>	
</body>
